...
export function PLSpTable({ data, isThousands = true, fiscalYear = "2026" }: PLSpTableProps) {
    ...
    const renderTable = (dataType: 'budget' | 'prevYear') => {
        const field = dataType === 'budget' ? 'budget' : 'prevYearActual';
        const dataLabel = dataType === 'budget'
            ? { jp: `${currentYear}年度 予算` }
            : { jp: `${prevYear}年度 実績` };
        ...
        return (
            <div className="h-[700px] w-full overflow-y-auto">
                    <Table className="min-w-[1900px]">
                        ...
                        <TableBody>
                            {plRows.map((row, idx) => (
                                <TableRow key={`${row.department}-${row.code}-${row.subject}`} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                                    ...
                                    {MONTH_ORDER.map(m => {
                                        const val = (row.monthlyData[m]?.[field] || 0) as number;
                                    ...
                                    <TableCell className="text-right font-bold text-sm bg-blue-50/50 border-l border-blue-100 text-blue-900 italic">
                                        {(isThousands ? Math.floor(((row.totalFirstHalf[field] || 0) as number) / 1000) : (row.totalFirstHalf[field] || 0) as number).toLocaleString()}
                                    </TableCell>
                                    <TableCell className="text-right font-bold text-sm bg-blue-50/50 border-blue-100 text-blue-900 italic">
                                        {(isThousands ? Math.floor(((row.totalSecondHalf[field] || 0) as number) / 1000) : (row.totalSecondHalf[field] || 0) as number).toLocaleString()}
                                    </TableCell>
                                    <TableCell className="text-right font-bold text-sm bg-blue-100/30 border-l-2 border-blue-100 text-blue-900">
                                        {(isThousands ? Math.floor(((row.totalAnnual[field] || 0) as number) / 1000) : (row.totalAnnual[field] || 0) as number).toLocaleString()}
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
